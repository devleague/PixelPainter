...
function PixelPainter (width, height) {
  ...
  // Private methods
  this.buildGrid = function(columns, rows, gridEl, isColorSwatch) {
    for (var row = 0; row < rows; row++) {
      gridEl.append(this.buildRow(columns, row, isColorSwatch));
    }
  }; // end .buildGrid()
  
  this.buildRow = function(columns, rowNumber, isColorSwatch) {
  ...
  this.getColor = function(indx, pallet) {
    var colorSets = {
      greys: [
        '#000000',
        '#303030',
...
PixelPainter.prototype.controls = function() {
  ...
  // add Erase button
  var $eraseButton = $('<div>', {'id': 'erase-button', 'class': 'control-btn', 'text': 'Erase'});
...
PixelPainter.prototype.artboard = function() {
  ...
  $artboardGrid.data('numberOfColumns', this.width).data('numberOfRows', this.height);

  //applies color to artboard
  $artboardGrid.on('click mousedown', '.cell', function () {
    $(this).css({'background-color': self.currentColor}).data('color', self.currentColor);
  });
